package me.lewin.dellunabank;

import me.lewin.dellunabank.event.EventInteractGUI;
import me.lewin.dellunabank.event.EventPlayerData;
import me.lewin.dellunabank.gui.InventoryBank;
...
public class Main extends JavaPlugin
{
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!(command.getName().equalsIgnoreCase("cash"))) return true;
    ...
    private void reload(){
        Bukkit.getServer().getOnlinePlayers().forEach(player -> Reference.setPlayerBankData(player.getUniqueId()));
    }
    // 자동완성 이벤트
    @Override
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        if (!(command.getName().equalsIgnoreCase("cash"))) return null;
    ...
    private void onDisplayMoneyRank(Player player) {
        ...
        player.sendMessage("§7============ §x§f§f§d§7§0§0델루나 부자랭킹 §7==========");
